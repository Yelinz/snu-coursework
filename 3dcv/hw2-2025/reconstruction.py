...
def FindMissingReconstruction(X, track_i):
    ...
    """
    new_point = (
        (np.all(X == -1, axis=1)) & (track_i[:, 0] != -1) & (track_i[:, 1] != -1)
    ).astype(int)
    ...
    for i in range(n_cameras):
        for j in range(n_points):
            if track[i, j, 0] != -1:
                ...
                S[
                    2 * k : 2 * (k + 1),
                    7 * n_cameras + 3 * j : 7 * n_cameras + 3 * (j + 1),
                ] = 1
...
def RunBundleAdjustment(P, X, track):
    ...
    print(
        "Reprojection error {} -> {}".format(np.linalg.norm(err0), np.linalg.norm(err))
    )
