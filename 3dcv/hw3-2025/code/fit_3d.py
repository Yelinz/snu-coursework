...
from hw3 import drawBBox, drawKeypoints, drawSkeleton, exportMesh, drawMesh
...
def main(base_dir,
    ...
    for ind, img_path in enumerate(img_paths):
        ...
        if not exists(out_path):
            ...
            # rescale points due to image being half the size for some reason
            joints = joints / 2
            params['opt_j2d'] = params['opt_j2d'] / 2
            ...
            img_bbox = drawBBox(img.copy(), joints, [0, 0, 255])
            # Draw 2D Keypoints.
            img_truth = drawKeypoints(img_bbox, joints, [0, 255, 0])
            img_out_3 = drawKeypoints(img_truth, params['opt_j2d'], [255, 0, 0])
            cv2.imwrite(f"results/bbox_keypoints_{ind}.jpg", img_out_3)
            # Draw the skeleton(connected joints) using the predicted joint locations.
            img_skeleton = drawSkeleton(img.copy(), params['opt_j2d'], [0, 255, 0], [255, 0, 0])
            cv2.imwrite(f"results/skeleton_{ind}.jpg", img_skeleton)
            # Export predicted SMPL mesh as .obj file.
            mesh_path = f"results/mesh_{ind}.obj"
            exportMesh(mesh_path, params['verts'], params['faces'])
            # Draw the projected SMPL mesh on the image.
            img_mesh = drawMesh(img.copy(), params['verts'], params['faces'], [255, 0, 0])
            cv2.imwrite(f"results/mesh_{ind}.jpg", img_mesh)
            # Find the reconstruction error.
            error = reconstructionError(params['opt_j2d'], joints)
            _LOGGER.info('Reconstruction error: %.4f', error)
