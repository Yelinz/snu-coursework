...
class ProjectPoints(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.rt:
            ...
            IS = np.repeat(np.arange(self.v.r.size*2/3, dtype=np.int32), 3)
            JS = np.asarray(np.repeat(np.arange(self.v.r.size, dtype=np.int32).reshape((-1,3)), 2, axis=0), order='C').ravel()
